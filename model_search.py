...
class MixedOp(nn.Module):
    ...
    def forward(self, x, weights):
        res=[]
        for w, op in zip(weights, self.m_ops):
            out = op(x)
            res.append(w*out)
        return sum(res)
...
class Network(nn.Module):
    ...
    def __init__(self, embed_size,max_features, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3, switches_normal=[], switches_reduce=[], p=0.0):
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(embed_size, C_curr, 3, padding=1, bias=False),
...
#            cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, switches)
    ...
    def forward(self, input):
        ...
        input =  self.embedding(input).permute(0,2,1)
